...
describe("FilesCommand", () => {
  ...
  it("opens branch files URL when file path is not provided", async () => {
    ...
    repositoryService.getFilesUrlForBranch.mockReturnValue(
      "https://example.dev/files",
    );
    ...
    expect(repositoryService.getFilesUrlForBranch).toHaveBeenCalledWith(
      repository,
      "feature/abc",
    );
  ...
  it("opens file URL when file path is provided", async () => {
    ...
    repositoryService.getFilesUrlForFileOnBranch.mockReturnValue(
      "https://example.dev/file",
    );

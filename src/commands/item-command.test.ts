...
describe('ItemCommand', () => {
  ...
  it('opens work item directly when --id is provided', async () => {
    ...
    repositoryService.getWorkItemUrl.mockReturnValue('https://example.dev/item/100');
    ...
    expect(browserService.open).toHaveBeenCalledWith('https://example.dev/item/100');
  ...
  it('throws when no PR exists for branch', async () => {
    ...
    await expect(command.execute({ directory: '/repo' })).rejects.toBeInstanceOf(CliError);
  ...
  it('opens first linked work item when PR has linked items', async () => {
    ...
    repositoryService.getWorkItemUrl.mockReturnValue('https://example.dev/item/55');
    repositoryIdCache.resolve.mockImplementation(async (_root, fetcher) => fetcher());
    azureDevOpsClient.getRepositoryId.mockResolvedValue('repo-id');
    ...
    expect(repositoryService.getWorkItemUrl).toHaveBeenCalledWith(repository, '55');
    expect(browserService.open).toHaveBeenCalledWith('https://example.dev/item/55');
  });
